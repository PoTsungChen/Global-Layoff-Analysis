...
UPDATE stg_layoffs
SET percentage_laid_off = round(percentage_laid_off,4)
...
SELECT layoff_year, layoff_qtr, country, total_layoff, qtr_rank
FROM qtr_rank_cte
WHERE qtr_rank <= 2;

SELECT YEAR(date) AS layoff_year, datepart(quarter, date) AS layoff_quarter, company, total_laid_off
FROM stg_layoffs
WHERE YEAR(date) = 2022 AND 
        country = 'United States' AND
        (datepart(quarter, date) = 3 OR datepart(quarter, date) = 4)
...
-- correlation coefficient for Funds_raised vs percentage_laid_off
